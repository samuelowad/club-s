...
// export class ForceRestrictive1733852927407 implements MigrationInterface {
...
export class FixTypeCasting1733852927408 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        const tables = ['event', 'user'];
        ...
        // Drop and recreate the function to return integer
        await queryRunner.query(`
            
            CREATE OR REPLACE FUNCTION current_club_id()
            RETURNS INTEGER AS $$
            BEGIN
                -- Convert the text setting to integer
                RETURN current_setting('app.current_club_id', true)::TEXT::INTEGER;
            END;
            $$ LANGUAGE plpgsql STABLE;
        `);
        ...
        for (const table of tables) {
            ...
            // Create a single restrictive policy with proper type casting
            console.log(`Creating restrictive policy for table ${table}`);
            await queryRunner.query(`
                CREATE POLICY ${table}_tenant_policy ON "${table}"
                FOR ALL
